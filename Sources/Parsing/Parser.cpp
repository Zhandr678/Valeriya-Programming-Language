...
namespace val
{
	...
	Statement Parser::ConstructAST(TokenLabel flag)
	{
		...
		{
			next = 0;
			if (auto st = AnalyzeBlockOfStatement()) {
				stms.push_back(*st);
			}
			...
			else if (auto st = AnalyzeAssignmentStatement()) {
				stms.push_back(*st);
			}
			else {
	...
	static Expression NormalizeFieldCallExpr(const Expression& expr)
	{
		...
		Expression left = view.caller();
		Expression right = view.field();

		left = NormalizeFieldCallExpr(left);

		if (right.option_is_FieldCall())
		{
			auto right_view = right.view_FieldCall();

			Expression merged_left(
				FieldCallExpr,
				left,
				right_view.caller()
			);
			...
			return NormalizeFieldCallExpr(rotated);
		}

		if (not right.option_is_VarName() && not right.option_is_ArrayIndex())
		{
			throw std::logic_error("RHS is not VarInit or ArrayIndex");
		}
			...
			else if (next_token.label == TokenLabel::IDENTIFIER)
			{
				...
				if (next_token.label == TokenLabel::SYM_LPAR)
				{
					...
					if (next_token.label == TokenLabel::SYM_DOT)
					{
						...
						{
							throw ParserException("Expected Field Name After '.'", GetFileName(), ExprCallStmt, GetLine());
						}

						single_exprs.emplace_back(UnaryExpr, NormalizeFieldCallExpr(Expression(FieldCallExpr, Expression(FnCallExpr, args.begin(), args.end(), name_token.attr), *expr_after_dot)), "-");
					}
					else if (next_token.label == TokenLabel::SYM_LBRACKET)
					{
						...
						while (next_token.label == TokenLabel::SYM_LBRACKET)
						{
							...
							{
								throw ParserException("Expected Expression", GetFileName(), ExprCallStmt, GetLine());
							}
							...
							array_index = Expression(ArrayIndexExpr, array_index, *index_expr);
						...
						if (next_token.label == TokenLabel::SYM_DOT)
						{
							...
							single_exprs.emplace_back(UnaryExpr, NormalizeFieldCallExpr(Expression(FieldCallExpr, array_index, *expr_after_dot)), "-");
						}
						else {
							next--;
							single_exprs.emplace_back(UnaryExpr, array_index, "-");
						}
					}
					else {
						next--;
				...
				else if (next_token.label == TokenLabel::SYM_LBRACE)
				{
					...
					if (next_token.label == TokenLabel::SYM_DOT)
					{
						...
						single_exprs.emplace_back(UnaryExpr, NormalizeFieldCallExpr(Expression(FieldCallExpr, Expression(StructInitExpr, struct_inits.begin(), struct_inits.end(), name_token.attr), *expr_after_dot)), "-");
					}
					else {
						next--;
						single_exprs.emplace_back(UnaryExpr, Expression(StructInitExpr, struct_inits.begin(), struct_inits.end(), name_token.attr), "-");
					}
				}
		...
		while (next_token.label != flag1 && next_token.label != flag2)
		{
			...
			else if (next_token.label == TokenLabel::SYM_LBRACE)
			{
				...
				while (next_token.label != TokenLabel::SYM_RBRACE)
				{
					...
					if (not expr.has_value())
					{
						throw ParserException("Expected Expression Inside Initializer List", GetFileName(), ExprCallStmt, GetLine());
					}
				...
				if (next_token.label == TokenLabel::SYM_DOT)
				{
					Token field_token = GetNextPeeked();

					if (field_token.label != TokenLabel::IDENTIFIER)
					{
						throw ParserException("Expected Field Name After '.'", GetFileName(), ExprCallStmt, GetLine());
					}

					single_exprs.push_back(NormalizeFieldCallExpr(Expression(
						FieldCallExpr,
						Expression(InitListExpr, array_exprs.begin(), array_exprs.end()),
						Expression(VarNameExpr, field_token.attr)
					)));
					next_token = GetNextPeeked();
				}
			...
			else if (next_token.label == TokenLabel::IDENTIFIER)
			{
				...
				else if (next_token.label == TokenLabel::SYM_LBRACE)
				{
					...
					if (next_token.label == TokenLabel::SYM_DOT)
					{
						...
						single_exprs.emplace_back(NormalizeFieldCallExpr(Expression(
							FieldCallExpr,
							Expression(StructInitExpr, struct_inits.begin(), struct_inits.end(), name_token.attr),
							Expression(VarNameExpr, field_token.attr)
						)));
						next_token = GetNextPeeked();
					}
				...
				else if (next_token.label == TokenLabel::SYM_LBRACKET)
				{
					...
					if (next_token.label == TokenLabel::SYM_DOT)
					{
						...
						single_exprs.push_back(NormalizeFieldCallExpr(Expression(FieldCallExpr, array_index, *expr_after_dot)));
					}
					else {
						next--;
						single_exprs.push_back(array_index);
					}
				...
				else if (next_token.label == TokenLabel::SYM_LPAR)
				{
					...
					if (next_token.label == TokenLabel::SYM_DOT)
					{
						...
						single_exprs.push_back(NormalizeFieldCallExpr(Expression(
							FieldCallExpr, Expression(FnCallExpr, args.begin(), args.end(), name_token.attr), *expr_after_dot
						)));
						next_token = GetNextPeeked();
					}
					else if (next_token.label == TokenLabel::SYM_LBRACKET)
					{
						...
						if (next_token.label == TokenLabel::SYM_DOT)
						{
							...
							single_exprs.push_back(NormalizeFieldCallExpr(Expression(
								FieldCallExpr, array_index, *expr_after_dot
							)));
						}
						else {
							next--;
				...
				else if (next_token.label == TokenLabel::SYM_DOT)
				{
					...
					single_exprs.push_back(NormalizeFieldCallExpr(Expression(
						FieldCallExpr, Expression(VarNameExpr, name_token.attr), *expr_after_dot
					)));
	...
	std::optional<Statement> Parser::AnalyzeBlockOfStatement()
	{
		...
		ResetLookAhead();
		auto block_body = ConstructAST(TokenLabel::SYM_RBRACE);
		...
		while (next_token.label != TokenLabel::SYM_SEMICOLON)
		{
			...
			if (next_token.label == TokenLabel::SYM_LBRACKET)
			{
				...
				if (next_token.label == TokenLabel::SYM_LBRACKET) // If Someday implement multidim arrays: if -> while
				{
					...
					if (GetNextPeeked().label != TokenLabel::SYM_RBRACKET)
					{
						throw ParserException("Expected Closing ']'", GetFileName(), ArrayInitStmt, GetLine());
					}
				...
				if (next_token.label == TokenLabel::SYM_ASSIGN)
				{
					...
					{
						throw ParserException("Expected Expression After '='", GetFileName(), ArrayInitStmt, GetLine());
					}

					array_init.view_ArrayInit().update_init_expr(*assign_expr);
					next_token = GetNextPeeked();
				}
				if (next_token.label == TokenLabel::SYM_COMMA)
				{
					var_inits.push_back(array_init);
					next_token = GetNextPeeked();
					ResetLookAhead();
					continue;
				}
				else if (next_token.label == TokenLabel::SYM_SEMICOLON)
				{
					...
					throw ParserException("Expected Semicolon", GetFileName(), ArrayInitStmt, GetLine());
				}
			}
	...
	std::optional<Statement> Parser::AnalyzeAssignmentStatement()
	{
		...
		while (next_token.label != TokenLabel::SYM_ASSIGN)
		{
			...
			{
				throw ParserException("Expected Semicolon", GetFileName(), ExprCallStmt, line);
			}
		...
		{
			throw ParserException("Expected Expression", GetFileName(), AssignmentStmt, GetLine());
		}
		...
		if (GetNextPeeked().label != TokenLabel::SYM_SEMICOLON)
		{
			throw ParserException("Expected Semicolon", GetFileName(), AssignmentStmt, GetLine());
		}
